...
public class GroupController {
    ...
    public ResponseEntity<?> addGroup(@Valid @RequestBody GroupAddDTO dto) {
        try {
            ExceptionObject exceptionObject = new ExceptionObject();
            Map<String, String> errorMap = new HashMap<>();
            int errorCode = HttpStatus.INTERNAL_SERVER_ERROR.value();
            exceptionObject.setCode(errorCode);
            if (groupService.findGroupByGroupName(dto.getGroupName()) != null) {
                errorMap.put("exception", "Nhóm đã tồn tại!");
    ...
    public ResponseEntity<?> editGroup(@PathVariable("id") int id, @Valid @RequestBody GroupUpdateDTO dto) {
        try {
